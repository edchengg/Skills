...
#     expname="gpt-oss-sdg-math-with-python",
...
generate(
    ctx=wrap_arguments(
        ...
        "++inference.temperature=1.0 "
        "++inference.top_p=1.0 "
        "++prompt_config=judge/math-judge "
        "++inference.endpoint_type=text "
        "++code_tags=gpt-oss "
        "++chat_template_kwargs.reasoning_effort=high "
    ),
    ...
    model="openai/gpt-oss-120b",
    cluster=cluster,
    expname=f"gptoss-judge",
    server_type='vllm',
    num_chunks=1,
    # can customize the number of GPUs used
    server_gpus=8,
    input_dir="/lustre/fsw/portfolios/llmservice/users/yachen/AceMath/Skills/gpt-oss-sdg/with-python/acereasonv3_lowpassrate",
    output_dir="/lustre/fsw/portfolios/llmservice/users/yachen/AceMath/Skills/gpt-oss-sdg/with-python/acereasonv3_lowpassrate-judge",
    dependent_jobs=0,
    starting_seed=0,
    num_random_seeds=32,
)