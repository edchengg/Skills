...
for benchmark in ["aime25"]:
    eval(
        ...
        output_dir=output_dir + model_name + "/with-python-json",
        # Use custom generation task that handles JSON tool calls
        # Use full path so it can be imported
        generation_module="/lustre/fsw/portfolios/llmservice/projects/llmservice_fm_text/users/yachen/AceMath/Skills/scripts/tool_call_json_task.py",
    )
